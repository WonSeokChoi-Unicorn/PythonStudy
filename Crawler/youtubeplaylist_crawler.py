...
youtubeplaylists = [
    "https://www.youtube.com/playlist?list=PLGK3hrw8eGO5ZCsN6SCj9jAWIKdxItN9E"
                   ]
...
def crawlingiframemake():
    ...
    for listurl in youtubeurls:
        ...
        # meta uploadDate 찾기
        dateinfo = iframeSoup.find('meta', attrs = {"itemprop" : "datePublished"})['content']

        # 날짜 문자열을 datetime 객체로 변환
        # %z는 시간대 오프셋을 포함하는 형식입니다.
        dt_obj = datetime.strptime(dateinfo, "%Y-%m-%dT%H:%M:%S%z")

        # datetime 객체를 UTC로 변환
        # tz=None은 시스템의 기본 시간대를 사용합니다.
        dt_obj_utc = dt_obj.astimezone(tz=None)

        # datetime 객체를 YYYYMMDDHHMMSS 형식으로 변환
        date_str = dt_obj_utc.strftime("%Y%m%d%H%M%S")
        # Dataframe에 추가
        pldf = pldf.append({'Date' : date_str, 'URL' : listurl}, ignore_index=True)
    # 날짜 오름차순으로 정렬
    ...
    for tempdf in pldf.itertuples():
        ...
        # " - YouTube"를 삭제 처리
        title = title.replace(" - YouTube", "") + " - " + tempdf.Date[0:3 + 1] + "/" + tempdf.Date[4:5 + 1] + "/" + tempdf.Date[6:7 + 1] + " " + tempdf.Date[8:9 + 1] + ":" + tempdf.Date[10:11 + 1] + ":" + tempdf.Date[12:13 + 1]
