...
if os.path.isfile(nowDate.strftime("%Y-%m-%d") + "_youtubeplaylist.txt"):
    ...
    fw = open(
        nowDate.strftime("%Y-%m-%d") + "_youtubeplaylist.txt",
        mode="at",
        encoding="utf-8",
    )
else:
...
def crawlingiframemake():
    ...
    for listurl in youtubeurls:
        ...
        # meta uploadDate 찾기
        dateinfo = iframeSoup.find("meta", attrs={"itemprop": "datePublished"})[
            "content"
        ]
        ...
        # Dataframe에 추가
        pldf = pd.concat(
            [pldf, pd.DataFrame([{"Date": date_str, "URL": listurl}])],
            ignore_index=True,
        )
    # 날짜 오름차순으로 정렬
